...
public class LottieView implements PlatformView, MethodChannel.MethodCallHandler {
    ...
    void create(Map<String, Object> args) {
        ...
        if (args.get("filePath") != null) {
            ...
            Log.d("LottieView filePath", key);
            animationView.setAnimation(key);
        }

        if (args.get("imagesRes") != null) {
            String key = mRegistrar.lookupKeyForAsset(args.get("imagesRes").toString());
            Log.d("LottieView", key);
            animationView.setImageAssetsFolder(key);
        }

        if (args.get("hardwareAcceleratedMode") != null) {
            boolean hardwareAcceleratedMode = Boolean.parseBoolean(args.get("hardwareAcceleratedMode").toString());
            if (hardwareAcceleratedMode) {
